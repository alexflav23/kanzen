...
- **`transaction_categories`** — `id, owner_id, parent_id null, name, kind ('income'|'expense'|'transfer')`. **Income** is first-class via `kind='income'` with seeded sub-categories **salary / dividend / rental / interest / other** — these feed F17 net cashflow and **F38** tax derivation (so salary/dividends come from real categorised credits, not manual entry).
