...
- **W9.6 — Guided inbox tour (in-theme, first-run).** A demo-grade interactive walkthrough — built *after* the W9 feature set is functionally complete (we finish the features before demoing them). Spec in §20. Tests: `GuidedTour.test` (step model + persistence) + `inbox-tour.spec` (Playwright: spotlight anchors the right element, advances, completes, persists, replays) + axe (focus-trap + `aria-live`, both themes).
...
## 20. Guided inbox tour (W9.6 — in-theme first-run walkthrough)
**Goal.** A high-quality, Apple-grade *interactive* tutorial that teaches the keystone feature by driving the real inbox — not a slideshow. It is the "first signup tutorial" for the inbox, fully in warm-paper theme (light + dark), using the existing tokens, Card/Pill/AgentRibbon language and restrained motion. Demo-grade; built only once W9.1–W9.4 are functionally complete.

**Primitive — `<GuidedTour>` (reusable).** A small, surface-agnostic overlay so Tasks/Wealth can adopt it later:
- **Step model (declarative):** `{ id, anchor: cssSelector | (() => Element), title, body, placement: 'top'|'bottom'|'left'|'right'|'center', spotlightPadding?, beforeEnter?: () => void, allowInteraction?: boolean }[]`. `beforeEnter` lets a step set up app state (e.g. *select the Ocado thread* so the proposal is on-screen).
- **Render:** a dimmed full-screen scrim (token `--overlay`) with a cut-out **spotlight** around the anchored element (SVG mask or 4-rect frame, rounded to the element's radius + padding) and a themed **callout card** (bgElev, shadow, indigo accent header, `完`-mark watermark) tethered to the anchor with a small caret; auto-flips placement to stay in viewport; repositions on scroll/resize (`ResizeObserver`).
- **Controls:** Back / Next / "Skip tour" + step dots ("2 of 5"); `→`/`Enter` next, `←` back, `Esc` skip. Final step CTA "Got it".
- **A11y:** focus-trapped to the callout; `role="dialog"` + `aria-labelledby`; step changes announced via `aria-live="polite"`; scrim `aria-hidden` siblings; full keyboard path; honors `prefers-reduced-motion` (cross-fade only, no slide).
- **Persistence:** `useGuidedTour(key)` → first-run gated per **user** (persist `kanzen.tour.<key>.done` keyed by user id; server-side `user_preferences` later, localStorage for now). Dismiss or complete sets done; never auto-shows again.
- **Replay:** a `?` / "Take the tour" affordance in the inbox header re-runs it on demand (ignores the done flag).

**The inbox script (`inboxTourSteps`).** Runs against seeded threads, read-only — it never confirms a proposal or sends mail:
1. **Welcome** (center): "This is your household inbox. Mail forwarded to `wardian@kanzen.family` lands here as a thread." 2. **A thread** (anchor: first `thread-row`; `beforeEnter` ensures the Ocado thread): "Every email becomes a thread — sender, subject, and whether it needs you." 3. **The agent proposal** (`beforeEnter` selects the thread; anchor: `agent-proposal`): "Kanzen reads the email *and its attachment* and proposes an action — here, logging the receipt. The % is its confidence." 4. **You decide** (anchor: the Confirm button): "**Confirm** creates the receipt or calendar event, linked back to this email. Kanzen never moves money — receipts go for approval." 5. **Assign** (anchor: the assignee control): "Hand a thread to a person — they'll see it in *Assigned to me*." 6. **Collaborate** (anchor: internal notes / "turn into task"): "Leave an internal note (never sent to the sender) or turn the thread into a task. When it's handled, mark it done and it leaves the queue." Final CTA "Got it".

**Acceptance.** First login to the inbox auto-opens the tour; spotlight precisely frames each anchor and repositions on resize; Next/Back/dots/keyboard all work; Skip and Complete both persist (no re-show on reload); the header replay re-opens it; nothing is mutated by the tour; axe-clean with focus-trap + `aria-live` in both themes; visual-diff goldens for two representative steps.
