...
## 4. API
- **NL query**: `POST /api/nl-query` — Claude (Bedrock) translates the prompt + the permission context into a **structured query** over F28/F29's model, executes it **read-only**, returns results + the interpreted query (transparent, correctable). Never mutates; never bypasses F02.
...
## 5. UI / screens & states
- **NL query bar** (in Insights/Search): ask in English → see the interpreted structured query (editable) + results; "save as smart filter" (F28).
