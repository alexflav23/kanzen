...
| F15 | Bills & recurring schedule | C | F03, F09 | ✅ | 🚧 backend done (bills CRUD, ±15% variance flag, seed, finance authz, tests); calendar reminders + web deferred |
| F16 | Payment methods & Pay queue (never moves money) | C | F15 | ✅ | 🚧 backend done (methods, schedule, queue, mark-paid MANUAL-only → 409 for auto; tests); web deferred |
