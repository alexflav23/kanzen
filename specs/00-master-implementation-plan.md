...
| F14 | Reconciliation engine (**single-spend / dedup association**) | C | F12, F13 | ✅ | 🚧 backend done (txn↔receipt match, single-spend 409 guard, unmatched list, finance authz, tests) **+ reconciliation state surfaced read-only in the web Transactions tab**; auto-suggest + split/refund/transfer + match-UI deferred |
...
| F29 | Dashboard, Insights & reporting | C | F18, F23 | ✅ | ✔️ Done (sandbox) — backend summary (authz/scope-filtered counts, no leak) + web live at-a-glance + e2e **+ Insights registry-health surface (F23 web)**; spend-trend charts deferred |
...
| F41 | Liabilities & consolidated net worth | G | F39, F42, F37, F04/F20, F12, F40 | ✅ | ✔️ Done (sandbox) — backend (liability accounts; net worth = GL assets + investments@market − liabilities, per-entity/consolidated, computed + snapshotted; Principal-private hard 403) + ITs **+ web net-worth surface (hero + entity scope selector, live-wired)**. Deferred: illiquid-asset valuation pull-in (F04/F20), FX display ccy |
