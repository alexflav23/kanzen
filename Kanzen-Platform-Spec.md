...
- **System-wide extensibility (v6.4).** The asset "trunk + freehand" model (typed core + JSONB `attributes` + templates) is **generalised to every object** — vendors, list items/products, people, properties, documents: ad-hoc **custom fields**, **polymorphic tags**, and **user-definable infinite taxonomies** (many category trees, not just the asset one). Mechanism stays **JSONB + Postgres trees** (not EAV/Avro). List items gain a product/vendor **URL** for "we're out of X, here's the link, buy it" requests. Detail in `specs/F33`.
- **Event backbone + Products (v6.5).** A unified **domain-event queue** — transactional outbox → **Apache Pulsar** (mirroring Hypervolt's stack) — underpins notifications/**APNs+FCM push**, search indexing, ledger posting and audit; "notify on task completion/comment, push to a staff member" becomes a subscription (**F34**). A **Products/stock** domain (consumables: preferred spec + preferred vendors + buy links + in/low/out stock) feeds Lists for "we're out of eggs, the Burford Browns, buy from Harrods" (**F35**). Tasks confirmed **native** (event-driven comments/notifications via F34; Vikunja-headless evaluated and declined).
- **NL search, predictive replenishment, attribute-level RBAC & currencies (v6.6).** A natural-language **search box** (F28 + F32) answers domain questions — "how many guitars do I have?", "how much did I pay for them?", "when did I last buy shoes?" — via Claude → a **validated, permission-filtered, read-only structured query**. **Predictive replenishment** (**F36**) learns purchase cadence from transactions to pre-empt run-outs. **RBAC is attribute-level** (F02/F33) — e.g. service an item **without seeing its price**. **Currencies & FX** (**F37**): native amounts stay truth, with dated, labelled conversions for cross-currency rollups — **resolving §19 #8**.
...
- **Trunk + freehand.** Every object has a mandatory typed core (the trunk); beyond it, **custom JSONB fields, polymorphic tags and user-definable infinite taxonomies** let anything be captured without a schema change — from categorising porcelain to a one-off "we're out of Dove Shower Gel, here's the link" (F33).
- **Everything emits events.** Every meaningful action publishes a **unified, serialized domain event** onto one queue; consumers react — notifications/push, search indexing, ledger posting, audit — so a new reaction ("ping me when a task is completed or commented") is a *subscription*, not new plumbing (F34).
...
Every asset has universal fields: `id`, `owner_id`, `title`, `description`, `category_id`, `tracking_mode`, `quantity`, `acquisition_date`, `acquisition_cost`, `acquisition_currency`, `merchant_id`, `ownership_status`, `condition_status`, `location_id`, `custody_status`, `notes`, `created_at`, `updated_at`, `deleted_at`. **Vertical-specific attributes** are stored as a **JSONB column on the asset**, validated against typed **category templates** (a definitions catalog used for validation + per-vertical UI) — so verticals not yet imagined are added without schema changes. (v6.2: JSONB rather than an EAV value table — see `specs/F04`.) Templates for guitars, glassware, porcelain, clothing/shoes, furniture, watches/jewellery, **art** and vehicles are in **Appendix C**. First-build verticals (from the prototype): watches, art, guitars, furniture, clothing, porcelain, glassware, vehicles.
...
8. **Cross-currency rollups — resolved (F37).** Native amounts stay truth; converted totals (e.g. net worth) are shown **explicitly + dated/labelled** in a chosen display currency with a per-currency breakdown, using stored daily FX snapshots + rate-at-transaction-time. Remaining sub-choice: the **FX rate provider** (ECB free vs commercial) — a SETUP item.
...
- **Assets:** `assets` (with a `jsonb` attributes column), `asset_groups`, `asset_group_members`, `collections`, `collection_members`, `tags`, `asset_tags`, `categories` (self-referencing tree), `category_templates` + `asset_attribute_definitions` (template/validation catalog — no per-value table; values are JSONB on `assets`), `asset_events`, `asset_comments`, `asset_valuation_snapshots`.
