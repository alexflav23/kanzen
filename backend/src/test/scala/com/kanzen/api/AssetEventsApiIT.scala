...
/** F19 — lifecycle events: Manager logs + reads the timeline + lifetime cost;
  * Staff have no registry access (403); invalid type rejected. */
object AssetEventsApiIT extends IOSuite {
  ...
  test("an invalid event type is rejected (400)") { xa =>
    newAsset(xa).flatMap(id => AssetEvents.log(xa, lorna, id, LogReq("banana", None, None, None)))
      .map(r => expect(r.left.exists(_._1.code == 400)))
  }

  test("logging against a non-existent asset is 404") { xa =>
    AssetEvents.log(xa, lorna, UUID.randomUUID(), LogReq("cleaned", None, None, None))
