...
/** F25 pipeline · F26 inbox · F27 trust. Emails are classified → proposed; financial/asset
  * categories can never be AUTO-executed (locked); human confirm is the review path. */
object AgentApiIT extends IOSuite {
  ...
  test("F25/F26 — a delivery email classifies + proposes actions; human confirm executes one") { xa =>
    for {
      ing   <- Agent.ingest(xa, lorna, IngestReq("deliveries@kanzen.family", "amazon", "Your order has been dispatched · delivery Tue")).map(_.toOption.get)
  ...
  test("F27 — a financial (Receipt) action can never be auto-executed (409); human confirm is allowed") { xa =>
    for {
      ing  <- Agent.ingest(xa, lorna, IngestReq("accounts@kanzen.family", "Selfridges", "Your receipt for the purchase")).map(_.toOption.get)
