...
/** F24 — legacy onboarding (AC1) + non-destructive, reversible merge (AC3) / split (AC4), proving the invariant that no
  * restructure silently destroys data (AC5).
  */
object RestructureApiIT extends IOSuite {
  ...
  private def legacy(xa: Transactor[IO], title: String, cost: Long): IO[UUID] =
    Restructure
      .legacyCreate(xa, lorna, LegacyCreateReq(title, None, Some(cost), Some("GBP"), None, None))
      .map(_.toOption.get.id)

  test("AC1 — legacy create stores an uncertainty note and is valid without a receipt") { xa =>
    for {
      c <- Restructure.legacyCreate(
        xa,
        lorna,
        LegacyCreateReq("Grandmother's clock", None, Some(25000L), Some("GBP"), None, Some("Inherited c. 1990"))
      )
