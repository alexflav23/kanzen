...
object ProductsApiIT extends IOSuite {
  ...
  test("F36 — forecast predicts the next purchase from history") { xa =>
    val dates =
      List(LocalDate.now.minusDays(60), LocalDate.now.minusDays(40), LocalDate.now.minusDays(20)).map(_.toString)
    Products.forecast(xa, lorna, ForecastReq(dates, Some(30))).map {
      case Right(f) =>
        expect(f.avgIntervalDays.contains(20.0)) and expect(f.predictedNext.isDefined) and expect(
          f.dueSoon
        ) // ~20d cadence, next within 30d
