...
/** F37 — currencies & FX overlay: convert at rate-on-date (AC2/AC3 rollup with per-currency
  * breakdown + dated-estimate label), nearest-prior selection, missing-rate handling. */
object FxApiIT extends IOSuite {
  ...
  test("currencies lists the active registry") { xa =>
    Fx.currencies(xa, toby).map(_.toOption.get).map(cs => expect(cs.map(_.code).toSet.subsetOf(Set("GBP","SGD","USD","EUR","CHF")) && cs.exists(_.code == "GBP")))
  }
  ...
  test("missing rate → 422 on convert; listed under unconvertible in a rollup (no silent blending)") { xa =>
    for {
      ...
      roll <- Fx.rollup(xa, toby, RollupReq("GBP", Some(on), List(Line("USD", 100000L), Line("JPY", 5000L)))).map(_.toOption.get)
