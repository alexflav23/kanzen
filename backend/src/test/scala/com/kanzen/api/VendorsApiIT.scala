...
/** F09 — vendors: selectability by insurance (AC1), property scope (AC4),
  * create + approve (AC5), staff-denied writes (AC6). Seeded directory (V2_29). */
object VendorsApiIT extends IOSuite {
  ...
  test("AC1 — only approved + insured vendors are selectable (expired insurance blocked)") { xa =>
    Vendors.selectable(xa, lorna, wardian).map {
      case Right(vs) => expect(vs.exists(_.name == "Thames Plumbing")) and expect(!vs.exists(_.name == "Volt Electrics")) // expired
  ...
  test("AC5 — Manager creates a vendor and approves it for a property") { xa =>
    for {
      created  <- Vendors.create(xa, lorna, Vendors.CreateReq("New Plumber", Some("business"), Some("plumber"), None, None, None)).map(_.toOption.get)
  ...
  test("AC6 — Staff cannot create vendors (403)") { xa =>
    Vendors.create(xa, marcia, Vendors.CreateReq("Nope", None, None, None, None, None)).map(r => expect(r.left.exists(_._1.code == 403)))
  }

  test("Manager sees the full directory across properties") { xa =>
    Vendors.list(xa, lorna).map(_.toOption.exists(vs => vs.exists(_.name == "Thames Plumbing") && vs.exists(_.name == "Marina Aircon")))
      .map(expect(_))
  }
}
