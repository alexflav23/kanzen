...
object PropertyCrudIT extends IOSuite {
  ...
  test("manager creates a property; staff cannot (403)") { xa =>
    for {
      created <- Properties.create(
        xa,
        manager,
        Properties.CreateReq("Cotswolds Cottage", Some("Glos"), Some("GB"), Some("house"), Some("owned"), "GBP")
      )
  ...
  test("patch updates particulars") { xa =>
    for {
      created <- Properties
        .create(xa, manager, Properties.CreateReq("Tmp", None, None, None, None, "GBP"))
        .map(_.toOption.get)
      patched <- Properties.patch(
        xa,
        manager,
        created.id,
        Properties.PatchReq("Renamed Villa", Some("Nice"), Some("FR"), Some("house"), Some("owned"))
      )
    ...
    for {
      created <- Properties
        .create(
          xa,
          manager,
          Properties.CreateReq("To Archive", None, Some("GB"), Some("apartment"), Some("owned"), "GBP")
        )
        .map(_.toOption.get)
