...
/** F18 / ADR-001 — double-entry GL: balanced-or-rejected, derived balances, reversing
  * corrections, Principal-private. */
object LedgerApiIT extends IOSuite {
  ...
  private def accounts(xa: Transactor[IO]): IO[(UUID, UUID)] =
    for {
      a <- Ledger.createAccount(xa, toby, AccountReq(s"expense:groceries:${UUID.randomUUID()}", "Groceries", "expense", "GBP")).map(_.toOption.get)
      b <- Ledger.createAccount(xa, toby, AccountReq(s"asset:cash:${UUID.randomUUID()}", "Cash", "asset", "GBP")).map(_.toOption.get)
    } yield (a.id, b.id)
  ...
  test("a balanced expense posts; derived balances reflect the splits") { xa =>
    for {
      ...
      posted   <- Ledger.post(xa, toby, PostReq("expense", Some("Waitrose"), None, List(SplitIn(expenseAcc, 4200L, None), SplitIn(cashAcc, -4200L, None))))
  ...
  test("an unbalanced transaction is rejected (400)") { xa =>
    for {
      ab <- accounts(xa)
      (a, b) = ab
      bad <- Ledger.post(xa, toby, PostReq("expense", None, None, List(SplitIn(a, 4200L, None), SplitIn(b, -4000L, None))))
  ...
  test("reversing a transaction nets the affected balances back to zero (immutable correction)") { xa =>
    for {
      ab <- accounts(xa)
      (a, b) = ab
      posted  <- Ledger.post(xa, toby, PostReq("expense", None, None, List(SplitIn(a, 1000L, None), SplitIn(b, -1000L, None)))).map(_.toOption.get)
  ...
  test("the ledger is Principal-only: Manager is denied (403) on post + balance") { xa =>
    for {
      ab <- accounts(xa)
      (a, b) = ab
      post <- Ledger.post(xa, lorna, PostReq("expense", None, None, List(SplitIn(a, 100L, None), SplitIn(b, -100L, None))))
