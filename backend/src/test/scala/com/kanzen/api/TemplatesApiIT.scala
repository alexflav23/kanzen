...
/** F22 — category templates: seeded schema readable; template validation wired into
  * asset create (required + typed keys enforced; unknown keys allowed). */
object TemplatesApiIT extends IOSuite {
  ...
  test("the seeded watch template is readable") { xa =>
    Templates.schema(xa, toby, "watch").map {
      case Right(fields) => expect(fields.exists(_.key == "serial")) and expect(fields.exists(f => f.key == "case_mm" && f.fieldType == "number"))
  ...
  test("a Principal creates a template; missing required + wrong type are rejected on asset create") { xa =>
    for {
      c <- cat(xa)
      _ <- Templates.create(xa, toby, TemplateReq("rare_book", Some("Rare book"),
      ...
      ok      <- Assets.create(xa, toby, asset(c, "rare_book", Json.obj("title_year" -> Json.fromInt(1812), "binding" -> Json.fromString("leather"), "extra" -> Json.fromString("freehand"))))
      missing <- Assets.create(xa, toby, asset(c, "rare_book", Json.obj("binding" -> Json.fromString("leather")))) // missing required title_year
      wrong   <- Assets.create(xa, toby, asset(c, "rare_book", Json.obj("title_year" -> Json.fromString("not a number")))) // wrong type
