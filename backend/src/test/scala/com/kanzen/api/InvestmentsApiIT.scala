...
/** F40 — cost-basis lots, holdings valuation with unrealised gain, and FIFO sells producing realised gains. Records
  * investments, never trades. Principal-private.
  */
object InvestmentsApiIT extends IOSuite {
  ...
  test("FIFO sell closes oldest lots first → realised gain = proceeds − closed cost basis") { xa =>
    for {
      ...
      _ <- Investments
        .buy(xa, toby, BuyReq(e, sec, 10.0, 100000L, Some(LocalDate.of(2026, 1, 1))))
        .map(_.toOption.get) // lot1 (older)
      _ <- Investments
        .buy(xa, toby, BuyReq(e, sec, 10.0, 120000L, Some(LocalDate.of(2026, 3, 1))))
        .map(_.toOption.get) // lot2
