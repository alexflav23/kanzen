...
/** F08 — shopping lists: Staff propose (non-recurring → needs_approval), Manager/Principal approve; Staff cannot
  * approve.
  */
object ListsApiIT extends IOSuite {
  ...
  test("a Staff-proposed non-recurring item needs approval; a recurring staple goes straight in") { xa =>
    for {
      ...
      prop <- Lists
        .addItem(xa, marcia, list.id, AddItemReq("Truffle oil", Some(1), Some(false), None))
        .map(_.toOption.get)
