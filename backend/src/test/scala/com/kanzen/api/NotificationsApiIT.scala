...
/** F34 — the notification-centre API: self-scoped inbox/read, subscriptions and device
  * registry. One user can never read or mark-read another's notifications. */
object NotificationsApiIT extends IOSuite {
  ...
  test("device registration validates platform (400) and is idempotent on the token") { xa =>
    for {
      ...
      dup <- Notifications.registerDevice(xa, marcia, Notifications.DeviceReq("fcm", "tok-123")) // re-register refreshes
  ...
  test("inbox shows fanned-out notifications; mark-read is self-scoped") { xa =>
    val env = Envelope("task.completed", Actor.system, Subject("task", UUID.randomUUID()), toby.userId, None, Json.obj())
    for {
      _      <- fanout(xa, env)
