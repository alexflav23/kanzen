...
/** F17 — expenses & approvals: threshold routing, over-threshold → Principal, deductibility
  * flags, finance carve-out (Staff 403). */
object ExpensesApiIT extends IOSuite {
  ...
  private def req(amount: Long, deductible: Boolean = false) =
    SubmitReq(Some("HVAC"), Some("quarterly"), amount, "GBP", None, None, Some(deductible), Some(false), Some("repairs"))
