...
/** F13 — receipts + line items: brand normalisation, line-item persistence, product-level spend (F32), and the finance
  * carve-out authz (Staff 403).
  */
object ReceiptsApiIT extends IOSuite {
  ...
  test("confirming a line item sets its category; unknown line → 404") { xa =>
    val req = CreateReq(
      ...
      Some(150L),
      Some("GBP"),
      List(LineIn(Some("Item 1"), Some(150L), Some("GBP"), None))
    )
    for {
