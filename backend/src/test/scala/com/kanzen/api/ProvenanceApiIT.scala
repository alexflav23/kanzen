...
object ProvenanceApiIT extends IOSuite {
  ...
  test("Manager can add + list warranties (registry)") { xa =>
    for {
      id  <- newAsset(xa)
      add <- Provenance.addWarranty(xa, lorna, id, AddWarrantyReq(Some("Audemars Piguet"), None, Some(LocalDate.now.plusYears(2))))
  ...
  test("insurance is Principal-only: Principal sets + reads; Manager is denied (403)") { xa =>
    for {
      id  <- newAsset(xa)
      set <- Provenance.setInsurance(xa, toby, id, SetInsuranceReq(insured = true, Some("Hiscox-2026"), Some("Hiscox"), Some(4_200_000L), Some(LocalDate.now.plusMonths(11))))
