...
object LocationTreeIT extends IOSuite {
  ...
  test("build a nested tree; a parent must be in the same property") { xa =>
    for {
      ...
      cab <- Locations
        .create(xa, manager, req(wardian, Some(living.id), "cabinet", "Drinks cabinet"))
        .map(_.toOption.get)
      cross <- Locations.create(
        xa,
        manager,
        req(singapore, Some(living.id), "shelf", "Bad")
      ) // parent in another property
