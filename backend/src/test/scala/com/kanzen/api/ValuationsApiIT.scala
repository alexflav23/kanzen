...
/** F20 — valuation snapshots (Principal-only) + F04 AC5: Manager sees the asset but its valuation fields are stripped
  * server-side.
  */
object ValuationsApiIT extends IOSuite {
  ...
  test("Staff cannot record a valuation (403)") { xa =>
    newAsset(xa)
      .flatMap(id => Valuations.record(xa, marcia, id, RecordReq("market", 1L, "GBP", None)))
  ...
  test("an invalid valuation kind is rejected (400)") { xa =>
    newAsset(xa)
      .flatMap(id => Valuations.record(xa, toby, id, RecordReq("banana", 1L, "GBP", None)))
