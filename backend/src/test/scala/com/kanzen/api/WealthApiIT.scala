...
/** Wave G (F42/F39/F41/F43) — entity-scoped double-entry, balance sheet that balances, net
  * worth = assets + investments − liabilities, and the Principal-private invariant (403). */
object WealthApiIT extends IOSuite {
  ...
  test("F39 — a balanced double-entry posts; an unbalanced one is rejected (422)") { xa =>
    for {
      ...
      ok   <- Wealth.post(xa, toby, PostReq(Some(e), "opening_balance", Some("seed"), Some(LocalDate.now()),
                List(SplitReq(cash, 100000L, None), SplitReq(eq, -100000L, None)))).map(_.toOption.get)
  ...
  test("F43 — the balance sheet balances: assets = liabilities + equity") { xa =>
    for {
      ...
      _    <- Wealth.post(xa, toby, PostReq(Some(e), "opening_balance", None, None, List(SplitReq(cash, 100000L, None), SplitReq(eq, -100000L, None)))).map(_.toOption.get)
      _    <- Wealth.post(xa, toby, PostReq(Some(e), "acquisition", None, None, List(SplitReq(prop, 500000L, None), SplitReq(mort, -500000L, None)))).map(_.toOption.get)
  ...
  test("F41 — net worth = GL assets + investments at market − liabilities (consolidated per entity)") { xa =>
    for {
      ...
      _    <- Wealth.post(xa, toby, PostReq(Some(e), "opening_balance", None, None, List(SplitReq(cash, 300000L, None), SplitReq(eq, -300000L, None)))).map(_.toOption.get)
      // loan drawdown: debit cash (+), credit liability (−) — liabilities are credit-normal
      _    <- Wealth.post(xa, toby, PostReq(Some(e), "transfer", None, None, List(SplitReq(cash, 50000L, None), SplitReq(mort, -50000L, None)))).map(_.toOption.get)
      // an investment lot for this entity, valued at market
      sec  <- Investments.createSecurity(xa, toby, Investments.CreateSecurityReq(s"SYM$e".take(12), "Test fund", None, None)).map(_.toOption.get.id)
