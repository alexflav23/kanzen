...
/** F38 — tax estimation + deductible report: Principal-only; report aggregates the
  * deductible/VAT flags from approved expenses. */
object TaxApiIT extends IOSuite {
  ...
  test("the deductible report sums deductible + VAT-reclaimable approved expenses (Principal-only)") { xa =>
    for {
      ...
      _      <- Expenses.submit(xa, lorna, SubmitReq(Some("Office supplies"), None, 12_000L, "GBP", None, None, Some(true), Some(true), Some("office")))
