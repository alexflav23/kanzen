...
object MaintenanceApiIT extends IOSuite {
  ...
  test("a plan due within its lead window is flagged dueSoon") { xa =>
    for {
      due  <- Maintenance.create(xa, lorna, CreateReq("Gutter clean", None, None, "annually", LocalDate.now.plusDays(3), Some(14))).map(_.toOption.get)
      far  <- Maintenance.create(xa, lorna, CreateReq("Repaint", None, None, "annually", LocalDate.now.plusDays(200), Some(14))).map(_.toOption.get)
    } yield expect(due.dueSoon) and expect(!far.dueSoon)
  }

  test("completing a service rolls next_due forward by frequency") { xa =>
    for {
      plan <- Maintenance.create(xa, lorna, CreateReq("Filter swap", None, None, "quarterly", LocalDate.now, Some(7))).map(_.toOption.get)
  ...
  test("Staff cannot create or complete (403)") { xa =>
    for {
      ...
      complete <- Maintenance.complete(xa, marcia, UUID.fromString("d0000000-0000-0000-0000-000000000001"), CompleteReq(None, None))
