...
/** F07 Path B — the outbound Google-Calendar push substrate: consumer (enqueue only when the tenant connected a
  * calendar) → durable queue → IO worker → the CalendarSync seam (which honours the F44 "Workspace connected"
  * contract). Fresh tenants + per-event-id assertions keep it parallelism-safe on the shared test DB.
  */
object CalendarSyncIT extends IOSuite {
  ...
  private def connectWorkspace(xa: Transactor[IO], t: UUID): IO[Unit] =
    WorkspaceRepo
      .connect(t, "kanzen.local", "sa@x.iam.gserviceaccount.com", s"dev:$t", List("cal"), admin)
      .transact(xa)
      .void
  ...
  test("the worker pushes a queued event through the seam and records the Google event id (Workspace connected)") {
    xa =>
