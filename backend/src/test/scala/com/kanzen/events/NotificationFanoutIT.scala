...
/** F34 AC1/AC5 — NotificationFanout matches subscriptions, **honours F02 at delivery**
  * (Staff never told about finance events), and is idempotent (no double-notify on redelivery). */
object NotificationFanoutIT extends IOSuite {
  ...
  test("AC5 — bill.variance_flagged reaches Principal + Manager but is F02-stripped from Staff") { xa =>
    val env = Envelope("bill.variance_flagged", Actor.system, Subject("bill", UUID.randomUUID()), toby, None, Json.obj())
    for {
      eid <- emitAndFanout(xa, env)
