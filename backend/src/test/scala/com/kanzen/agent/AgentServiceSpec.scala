...
class AgentServiceSpec extends AnyFreeSpec with Matchers {
  "AgentService.classify" - {
    "an invoice subject -> Bill / Invoice" in { AgentService.classify("SP Group · May invoice available") shouldBe "Bill / Invoice" }
    "a dispatch subject -> Delivery" in { AgentService.classify("Your Amazon order has been dispatched") shouldBe "Delivery" }
