...
/** F00/F05 — the blob store behind the evidence store. Originals are **write-once**
  * (immutable): `put` to an existing key is rejected. The in-memory impl backs tests +
  * local dev; the S3 impl (AWS SDK + LocalStack, real presign + SSE) wires in later
  ...
  def inMemory: IO[ObjectStore] =
    Ref.of[IO, Map[String, (String, Array[Byte])]](Map.empty).map { ref =>
      new ObjectStore {
        def put(key: String, contentType: String, bytes: Array[Byte]): IO[Unit] =
          ref.modify { m =>
            if (m.contains(key)) (m, IO.raiseError[Unit](new IllegalStateException(s"object exists (write-once): $key")))
