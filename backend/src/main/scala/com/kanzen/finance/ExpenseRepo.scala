...
object ExpenseRepo {
  ...
  def submit(ownerId: UUID, payee: Option[String], description: Option[String], amountMinor: Long, currency: String,
             propertyId: Option[UUID], categoryId: Option[UUID], deductible: Boolean, vatReclaimable: Boolean,
             taxCategory: Option[String], requestedBy: UUID): ConnectionIO[Expense] = {
    val status = ExpenseService.initialStatus(amountMinor, currency)
