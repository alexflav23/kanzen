...
object Jwks {
  /** In-memory keyset. Used by tests (the local **test-JWKS**) and as the empty
    * default until a real Cognito pool's JWKS endpoint is wired (the HTTP-fetching
    * impl lands with the dev pool — plan Phase 0/1, ⛔ items). */
