...
final case class Entity(id: UUID, name: String, kind: String, jurisdiction: Option[String], baseCurrency: String, parentEntityId: Option[UUID])
final case class WealthAccount(id: UUID, code: String, name: String, accountType: String, currency: String, balanceMinor: Long, subkind: Option[String])

/** Wave G (F42/F39/F41/F43) — entity-scoped chart of accounts, statements and net worth, all
  * derived from the F18 general ledger (`gl_splits` are the ledger). Principal-private. */
object WealthRepo {
  ...
  def createEntity(ownerId: UUID, name: String, kind: String, jurisdiction: Option[String], baseCurrency: String, parent: Option[UUID]): ConnectionIO[UUID] =
  ...
  def createAccount(ownerId: UUID, entityId: UUID, code: String, name: String, accountType: String, currency: String, subkind: Option[String]): ConnectionIO[UUID] =
    sql"""insert into gl_accounts (owner_id, entity_id, code, name, type, currency, subkind)
          values ($ownerId, $entityId, $code, $name, $accountType, $currency, $subkind) returning id""".query[UUID].unique
  ...
  def balanceByType(ownerId: UUID, entity: Option[UUID]): ConnectionIO[Map[String, Long]] =
    (fr"""select a.type, coalesce(sum(s.amount_minor), 0)
      ...
      .query[(String, Long)].to[List].map(_.toMap)
  ...
  def saveSnapshot(ownerId: UUID, entity: Option[UUID], assets: Long, liabilities: Long, net: Long, currency: String): ConnectionIO[UUID] =
