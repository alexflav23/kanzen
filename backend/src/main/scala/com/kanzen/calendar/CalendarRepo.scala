...
object CalendarRepo {
  ...
  def createNative(ownerId: UUID, title: String, on: LocalDate, category: String, propertyId: Option[UUID],
                   source: String, sourceId: Option[UUID]): ConnectionIO[UUID] =
  ...
  def bySource(source: String, sourceId: UUID): ConnectionIO[Option[UUID]] =
    sql"select id from calendar_event_refs where source = $source and source_id = $sourceId and deleted_at is null".query[UUID].option

  /** The merged view: native/Google events in range + read-only overlays from due tasks and
    * maintenance plans. Native = system-of-linkage; tasks/maintenance are derived, not stored. */
