...
/** F01 — resolve validated JWT claims to the canonical Kanzen principal: match the
  * Cognito sub (`login_identities`), else fall back to email (`users`); reject anyone
  * who isn't an **active** user. The DB role is authoritative; the JWT role claim is
  * only a hint. Real Cognito provisioning (auto-create + link on first login) lands
  * with the dev pool — for now the seeded household resolves by email. */
object Principals {
