...
/** F13 — receipts + line items. OCR/Bedrock parse is an adapter (deferred); this stores the parsed result. Brand
  * normalisation on each line powers product-level spend (F32).
  */
object ReceiptService {

  /** A stable brand/product key from a free-text line: the text before the first digit, lowercased and hyphenated.
...
object ReceiptRepo {
  ...
  private val liCols =
    fr"id, receipt_id, line_no, description, total_minor, currency, brand_norm, suggested_category, confirmed_category, status"
  ...
  def lines(receiptId: UUID): ConnectionIO[List[LineItem]] =
    (fr"select" ++ liCols ++ fr"from receipt_line_items where receipt_id = $receiptId order by line_no")
      .query[LineItem]
      .to[List]

  def lineExists(lineId: UUID, receiptId: UUID): ConnectionIO[Boolean] =
    sql"select exists(select 1 from receipt_line_items where id = $lineId and receipt_id = $receiptId)"
      .query[Boolean]
      .unique
