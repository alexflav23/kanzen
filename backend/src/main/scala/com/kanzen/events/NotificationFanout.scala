...
/** F34 — fans a domain event out to in-app notifications (+ push/email channels, mocked in sandbox) for every matching
  * subscription, **honouring F02 at delivery** (AC5): a recipient is never told about a subject they cannot read. The
  * fan-out is idempotent (dedup by event+user), so redelivery never double-notifies.
  */
object NotificationFanout extends Consumer {
  val name = "NotificationFanout"

  /** The F02 resource a recipient must be able to read to be told about this event. `None` ⇒ a platform event with no
    * subject gate (everyone subscribed may receive it).
    */
  private def gateResource(eventType: String): Option[String] =
    eventType.takeWhile(_ != '.') match {
      case "bill" | "expense" | "reconciliation" | "ledger" | "payment" | "bank" =>
        Some("bill") // finance family — Staff have none
...
/** The standard sandbox consumer set wired into the relay (more land with their features: SearchIndexer F28,
  * LedgerPoster F18, ReminderScheduler F11).
  */
object Consumers {
  val sandbox: List[Consumer] =
    List(NotificationFanout, com.kanzen.index.IndexConsumer, com.kanzen.calendar.CalendarSyncConsumer)
}
