...
/** F34 — the unified, schema-versioned domain-event envelope. Serialised into `event_outbox.payload`; every producer
  * emits this shape so any consumer can react without bespoke wiring. `owner_id`/`property_id` let consumers honour F02
  * at delivery.
  */
