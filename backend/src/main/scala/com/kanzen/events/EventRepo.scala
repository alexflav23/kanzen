...
/** F34 — transactional outbox: events written in the same tx as the domain change,
  * then relayed (to Pulsar in production; in-process [[Relay]] in sandbox). The relay marks
  * rows published; a crash before commit leaves them unpublished to resume next drain (AC3).
  */
object EventRepo {

  /** A raw outbox row; `payload` is the serialised [[Envelope]]. */
  final case class OutboxRow(id: UUID, eventType: String, aggregateType: String, aggregateId: Option[UUID], payload: Json)
