...
/** F18 / ADR-001 — the double-entry invariant: a transaction's signed splits must sum to zero, with at least two splits
  * and at least one non-zero amount. Balanced-or-rejected.
  */
...
object GlRepo {
  ...
  def splitsOf(transactionId: UUID): ConnectionIO[List[(UUID, Long, Option[String])]] =
    sql"select account_id, amount_minor, memo from gl_splits where transaction_id = $transactionId"
      .query[(UUID, Long, Option[String])]
      .to[List]
