...
/** F09 — vendors & contacts, with property-scoped approval (F02). Principal/Manager
  * write; Staff read only vendors approved for a property in their scope. A vendor is
  * assignable only while its insurance is current (selectable list). */
object Vendors {
  private type Out[A] = Either[(StatusCode, ApiError), A]

  final case class VendorView(id: UUID, name: String, `type`: String, trade: Option[String],
                              ndaUntil: Option[LocalDate], insuranceUntil: Option[LocalDate], rating: Option[BigDecimal])
  final case class VendorDetail(vendor: VendorView, approvedProperties: List[UUID])
  final case class CreateReq(name: String, `type`: Option[String], trade: Option[String],
                             ndaUntil: Option[LocalDate], insuranceUntil: Option[LocalDate], rating: Option[BigDecimal])
  final case class ApproveReq(propertyId: UUID)
  final case class OkResult(ok: Boolean)

  private def view(v: Vendor): VendorView = VendorView(v.id, v.name, v.`type`, v.trade, v.ndaUntil, v.insuranceUntil, v.rating)

  private val forbidden: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "forbidden", "no access to vendors"))
  ...
  def create(xa: Transactor[IO], p: Principal, req: CreateReq): IO[Out[VendorView]] = {
    val tx = Authz.authorizer(p.role).flatMap { authz =>
      ...
      else VendorRepo.insert(p.userId, req.name, req.`type`.getOrElse("business"), req.trade, req.ndaUntil, req.insuranceUntil, req.rating)
  ...
  def selectable(xa: Transactor[IO], p: Principal, propertyId: UUID): IO[Out[List[VendorView]]] = {
    val tx = for {
      authz  <- Authz.authorizer(p.role)
      scoped <- scopedIds(p)
      rows   <- if (authz.canRead("vendor") && scoped.contains(propertyId)) VendorRepo.selectableFor(propertyId) else List.empty[Vendor].pure[ConnectionIO]
    } yield
      if (!authz.canRead("vendor")) Left(forbidden)
  ...
  val listEndpoint: Endpoint[String, Unit, (StatusCode, ApiError), List[VendorView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "vendors").errorOut(err).out(jsonBody[List[VendorView]]).summary("Vendors (Staff see only their property's)")

  val selectableEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), List[VendorView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "vendors" / "selectable").in(query[UUID]("property")).errorOut(err).out(jsonBody[List[VendorView]])
  ...
  val detailEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), VendorDetail, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "vendors" / path[UUID]("id")).errorOut(err).out(jsonBody[VendorDetail]).summary("A vendor + its approved properties")
  ...
  val createEndpoint: Endpoint[String, CreateReq, (StatusCode, ApiError), VendorView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "vendors").in(jsonBody[CreateReq]).errorOut(err).out(jsonBody[VendorView]).summary("Add a vendor (Manager+)")

  val approveEndpoint: Endpoint[String, (UUID, ApproveReq), (StatusCode, ApiError), OkResult, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "vendors" / path[UUID]("id") / "properties").in(jsonBody[ApproveReq]).errorOut(err).out(jsonBody[OkResult])
  ...
  def serverEndpoints(a: Auth, xa: Transactor[IO]): List[ServerEndpoint[Any, IO]] = List(
    ...
    approveEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => { case (id, r) => approve(xa, p, id, r.propertyId) }),
  )

  val endpoints: List[AnyEndpoint] = List(listEndpoint, selectableEndpoint, detailEndpoint, createEndpoint, approveEndpoint)
}
