...
/** F25 email-agent pipeline + F27 trust model + F26 unified inbox. Mail is ingested, classified (rules in sandbox;
  * Bedrock in prod) and mapped to **proposed** actions — never auto-committed for financial/asset categories (F27 lock,
  * SPEC §10.3 / house rule). Human confirm/reject goes through the same Authorizer as any other write.
  */
object Agent {
  ...
  def ingest(xa: Transactor[IO], p: Principal, r: IngestReq): IO[Out[IngestResult]] =
    write(
      ...
      } yield IngestResult(emailId, category, ids)
    ).transact(xa)
  ...
  /** Human confirm — the review step. Allowed for any category (incl. locked financial ones); it's the human review the
    * lock requires. Goes through the agent-write authz path.
    */
      ...
      res <-
        if (!a.can(Actions.byKey("agent:use"))) (Left(forbidden): Out[Ok]).pure[ConnectionIO]
          ...
          cat match {
            ...
            case Some((_, status)) if status != "proposed" =>
              (Left(conflict(s"action already $status")): Out[Ok]).pure[ConnectionIO]
            case Some(_) =>
              (if (execute) AgentRepo.confirm(id) else AgentRepo.reject(id)).map(n => Right(Ok(n > 0)): Out[Ok])
          }
    } yield res
    tx.transact(xa)
  }

  /** F27 — attempt to AUTO-execute (no human review). Blocked (409) for locked financial/asset categories: those can
    * never be auto-committed, regardless of trust routing.
    */
            ...
            case Some((category, _)) =>
              TrustRepo.get(category).flatMap { routing =>
                if (!TrustService.canAutoExecute(category, routing.getOrElse("review")))
                  (Left(conflict(s"'$category' is locked to review — never auto-committed")): Out[Ok])
                    .pure[ConnectionIO]
  ...
  def setTrust(xa: Transactor[IO], p: Principal, r: SetTrustReq): IO[Out[TrustView]] =
    write(
      p,
      TrustRepo.set(r.category, r.routing).map(eff => TrustView(r.category, eff, TrustService.locked(r.category)))
    ).transact(xa)
  ...
  val ingestEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "agent" / "ingest")
    .in(jsonBody[IngestReq])
    .errorOut(err)
    .out(jsonBody[IngestResult])
    .summary("Ingest + classify an email → propose actions")
  val actionsEndpoint = sttp.tapir.endpoint.get
    .securityIn(bearer)
    .in("api" / "agent" / "actions")
    .in(query[String]("status").default("proposed"))
    .errorOut(err)
    .out(jsonBody[List[ActionView]])
    .summary("Agent actions by status (Triage)")
  val confirmEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "agent" / "actions" / path[UUID]("id") / "confirm")
    .errorOut(err)
    .out(jsonBody[Ok])
    .summary("Confirm (human review) an action")
  val rejectEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "agent" / "actions" / path[UUID]("id") / "reject")
    .errorOut(err)
    .out(jsonBody[Ok])
    .summary("Reject an action")
  val autoEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "agent" / "actions" / path[UUID]("id") / "auto-execute")
    .errorOut(err)
    .out(jsonBody[Ok])
    .summary("Attempt auto-execute (blocked for locked categories)")
  val trustEndpoint = sttp.tapir.endpoint.get
    .securityIn(bearer)
    .in("api" / "agent" / "trust")
    .errorOut(err)
    .out(jsonBody[List[TrustView]])
    .summary("Trust routing per category")
  val setTrustEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "agent" / "trust")
    .in(jsonBody[SetTrustReq])
    .errorOut(err)
    .out(jsonBody[TrustView])
    .summary("Set a category's routing (financial forced to review)")
  val inboxEndpoint = sttp.tapir.endpoint.get
    .securityIn(bearer)
    .in("api" / "inbox")
    .errorOut(err)
    .out(jsonBody[InboxView])
    .summary("Unified inbox counts")
