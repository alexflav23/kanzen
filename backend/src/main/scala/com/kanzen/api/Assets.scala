...
/** F04 — the asset registry. Principal-private with the Manager carve-out (F02): Staff
  * get 403 (asset = none); Manager/Principal read+write. Field-level stripping of
  * valuation fields is applied here once F20 adds them (mechanism: Authorizer.filterReadable). */
object Assets {
  private type Out[A] = Either[(StatusCode, ApiError), A]

  private val MODES = Set("unique", "grouped_quantity", "structured_set")

  final case class AssetView(id: UUID, title: String, maker: Option[String], categoryId: Option[UUID],
                             trackingMode: String, quantity: Int, ownershipStatus: String)
  final case class AssetDetail(id: UUID, title: String, maker: Option[String], categoryId: Option[UUID],
  ...
  final case class CreateReq(title: String, maker: Option[String], categoryId: UUID, vertical: Option[String],
                             ...
                             locationId: Option[UUID], attributes: Option[Json])

  private def view(a: Asset): AssetView = AssetView(a.id, a.title, a.maker, a.categoryId, a.trackingMode, a.quantity, a.ownershipStatus)
  private def detailOf(a: Asset): AssetDetail =
    AssetDetail(a.id, a.title, a.maker, a.categoryId, a.vertical, a.trackingMode, a.quantity, a.parentAssetId,
      a.acquisitionCostMinor, a.acquisitionCurrency, a.ownershipStatus, a.locationId, a.attributes)

  private val forbidden: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "forbidden", "no access to the registry"))
  ...
  val listEndpoint: Endpoint[String, (Option[UUID], Option[String]), (StatusCode, ApiError), List[AssetView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "assets").in(query[Option[UUID]]("category")).in(query[Option[String]]("q"))
      .errorOut(err).out(jsonBody[List[AssetView]]).summary("List assets (Principal-private; faceted by category/q)")

  val detailEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), AssetDetail, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "assets" / path[UUID]("id")).errorOut(err).out(jsonBody[AssetDetail])
