...
/** F17 — budgets, expenses & approvals. Manager submits + decides under threshold;
  * over-threshold routes to the Principal (per-jurisdiction native thresholds, no FX).
  * Deductibility/VAT flags ride along (F38). Finance carve-out (Staff none). */
object Expenses {
  private type Out[A] = Either[(StatusCode, ApiError), A]

  final case class ExpenseView(id: UUID, payee: Option[String], amountMinor: Long, currency: String, status: String, deductible: Boolean, vatReclaimable: Boolean)
  final case class SubmitReq(payee: Option[String], description: Option[String], amountMinor: Long, currency: String,
                             propertyId: Option[UUID], categoryId: Option[UUID], deductible: Option[Boolean],
                             vatReclaimable: Option[Boolean], taxCategory: Option[String])

  private def view(e: Expense): ExpenseView = ExpenseView(e.id, e.payee, e.amountMinor, e.currency, e.status, e.deductible, e.vatReclaimable)

  private val forbidden: (StatusCode, ApiError)  = (StatusCode.Forbidden, ApiError(403, "forbidden", "no access to expenses"))
  private val needsPrincipal: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "approval_threshold", "over-threshold approval requires the Principal"))
  ...
  def submit(xa: Transactor[IO], p: Principal, req: SubmitReq): IO[Out[ExpenseView]] = {
    val tx = Authz.authorizer(p.role).flatMap { authz =>
      ...
      else ExpenseRepo.submit(p.userId, req.payee, req.description, req.amountMinor, req.currency, req.propertyId,
        req.categoryId, req.deductible.getOrElse(false), req.vatReclaimable.getOrElse(false), req.taxCategory, p.userId)
  ...
  private def decide(xa: Transactor[IO], p: Principal, id: UUID, action: (UUID, UUID) => ConnectionIO[Int]): IO[Out[ExpenseView]] = {
    val tx = for {
  ...
  val submitEndpoint: Endpoint[String, SubmitReq, (StatusCode, ApiError), ExpenseView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "expenses").in(jsonBody[SubmitReq]).errorOut(err).out(jsonBody[ExpenseView]).summary("Submit an expense (Manager+; threshold-routed)")
  ...
  val listEndpoint: Endpoint[String, Option[String], (StatusCode, ApiError), List[ExpenseView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "expenses").in(query[Option[String]]("status")).errorOut(err).out(jsonBody[List[ExpenseView]]).summary("Expenses (finance read; ?status=)")

  val approveEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), ExpenseView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "expenses" / path[UUID]("id") / "approve").errorOut(err).out(jsonBody[ExpenseView]).summary("Approve (over-threshold → Principal)")

  val rejectEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), ExpenseView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "expenses" / path[UUID]("id") / "reject").errorOut(err).out(jsonBody[ExpenseView]).summary("Reject (over-threshold → Principal)")
