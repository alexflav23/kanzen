...
/** F20 — dated valuation snapshots on an asset. Principal-only: the canonical Manager-denied fields
  * (`asset.valuation_snapshots` + `market_value`/`insured_value`, F02). Current value = latest snapshot of each kind.
  */
object Valuations {
      ...
      val tx = for {
        ...
        res <-
          if (!authz.can(Level.Write, "asset", Some("valuation_snapshots")))
            ...
            ValuationRepo
              .add(assetId, req.kind, req.amountMinor, req.currency, req.source)
              .map(v => Right(view(v)): Out[ValuationView])
      } yield res
      tx.transact(xa)
    }
  }
  ...
  val recordEndpoint: Endpoint[String, (UUID, RecordReq), (StatusCode, ApiError), ValuationView, Any] =
    sttp.tapir.endpoint.post
      ...
      .in("api" / "assets" / path[UUID]("id") / "valuations")
      .in(jsonBody[RecordReq])
      .errorOut(err)
      .out(jsonBody[ValuationView])
  ...
  val listEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), List[ValuationView], Any] =
    sttp.tapir.endpoint.get
      ...
      .in("api" / "assets" / path[UUID]("id") / "valuations")
      .errorOut(err)
      .out(jsonBody[List[ValuationView]])
