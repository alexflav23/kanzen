...
/** F24 — legacy onboarding (relaxed-validation create) + non-destructive, auditable, reversible restructure (merge /
  * split). Originals are superseded + linked, never deleted; cost basis is reallocated explicitly and the before/after
  * is recorded so every reshape stays explainable.
  */
object Restructure {
  private type Out[A] = Either[(StatusCode, ApiError), A]

  final case class LegacyCreateReq(
      title: String,
  ...
  private val forbidden: (StatusCode, ApiError) =
    (StatusCode.Forbidden, ApiError(403, "forbidden", "no access to assets"))
        ...
        .flatMap {
          ...
          case true =>
            RestructureRepo
              ...
              .map(id => Right(Created(id)): Out[Created])
        }
        .transact(xa)
      ...
      val tx = for {
        ...
        res <-
          if (!w) (Left(forbidden): Out[MergeResult]).pure[ConnectionIO]
            ...
            val combined = sv + mc
            val before = Json.obj(
              "survivor" -> Json.obj("id" -> r.survivorId.asJson, "costMinor" -> sv.asJson),
              ...
              case Some((owner, cat, title, costOpt)) =>
                ...
                for {
                  children <- allocs.zipWithIndex.traverse { case (alloc, i) =>
                    RestructureRepo.createBare(
                      owner.getOrElse(p.userId),
                      s"$title (${i + 1}/${r.intoCount})",
                      cat,
                      Some(r.assetId),
                      Some(alloc),
                      None,
                  ...
                  after = Json.obj(
                    "children" -> children
                      .zip(allocs)
                      .map { case (id, a) => Json.obj("id" -> id.asJson, "costMinor" -> a.asJson) }
                      .asJson
                  )
  ...
  /** AC3/AC5 — reverse a merge (restore survivor cost + un-supersede merged) or a split (un-supersede parent +
    * supersede the children). Records a linked reversal op.
    */
      ...
      res <-
        if (!w) (Left(forbidden): Out[ReverseResult]).pure[ConnectionIO]
          ...
          op match {
            ...
            case Some((kind, _, outputs)) =>
              doReverse(opId, kind, outputs).map(rid => Right(ReverseResult(opId, rid, kind)): Out[ReverseResult])
          }
    } yield res
    tx.transact(xa)
  }
  ...
  private def doReverse(opId: UUID, kind: String, outputs: Json): ConnectionIO[UUID] = kind match {
    ...
    case "split" =>
      val children =
        outputs.hcursor.downField("children").values.toList.flatten.flatMap(_.hcursor.get[UUID]("id").toOption)
      val parentId =
        outputs.hcursor.downField("parent").get[UUID]("id").toOption // may be absent (split stores parent in `before`)
      for {
        _ <- children.traverse_(cid => RestructureRepo.supersedeNoSurvivor(cid))
        _ <- RestructureRepo
          .costBasisBefore(opId)
          .flatMap(b =>
            b.flatMap(_.hcursor.downField("parent").get[UUID]("id").toOption)
              .orElse(parentId)
              .traverse_(RestructureRepo.unsupersede)
          )
  ...
  val legacyEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "assets" / "legacy")
    .in(jsonBody[LegacyCreateReq])
    .errorOut(err)
    .out(jsonBody[Created])
    .summary("Create a legacy asset (relaxed validation)")
  val mergeEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "assets" / "merge")
    .in(jsonBody[MergeReq])
    .errorOut(err)
    .out(jsonBody[MergeResult])
    .summary("Merge two assets (lineage + cost preserved)")
  val splitEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "assets" / "split")
    .in(jsonBody[SplitReq])
    .errorOut(err)
    .out(jsonBody[SplitResult])
    .summary("Split an asset into children (cost allocated)")
  val reverseEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "restructure" / path[UUID]("opId") / "reverse")
    .errorOut(err)
    .out(jsonBody[ReverseResult])
    .summary("Reverse a restructure operation")

  def serverEndpoints(a: Auth, xa: Transactor[IO]): List[ServerEndpoint[Any, IO]] = List(
    legacyEndpoint
      .serverSecurityLogic(a.securityLogic)
      .serverLogic(p => (r: LegacyCreateReq) => legacyCreate(xa, p, r)),
