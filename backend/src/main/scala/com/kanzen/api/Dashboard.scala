...
/** F29 — the dashboard summary: cross-domain counts, each gated by what the principal may
  * read (Manager sees no registry assets; Staff see neither finance nor registry). No
  * totals leak past authz/scope. */
object Dashboard {
  ...
  def summary(xa: Transactor[IO], p: Principal): IO[Out[Summary]] = {
    val tx = for {
      ...
      pending <- if (authz.canRead("expense")) ExpenseRepo.list(Some("pending_approval")) else List.empty.pure[ConnectionIO]
      permits <- if (p.role != "staff" && authz.canRead("person")) PeopleRepo.expiringPermits(60) else List.empty.pure[ConnectionIO]
  ...
  val endpoint: Endpoint[String, Unit, (StatusCode, ApiError), Summary, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "dashboard").errorOut(statusCode.and(jsonBody[ApiError])).out(jsonBody[Summary])
