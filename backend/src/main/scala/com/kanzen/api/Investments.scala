...
/** F40 — investments & securities: cost-basis lots, a price database, holdings valuation with
  * unrealised gains, and FIFO sells with realised gains. **Records investments, never executes
  * trades.** Principal-private. */
object Investments {
  ...
  final case class BuyReq(entityId: UUID, securityId: UUID, quantity: Double, costBasisMinor: Long, acquiredOn: Option[LocalDate])
  final case class SellReq(entityId: UUID, securityId: UUID, quantity: Double, proceedsMinor: Long, on: Option[LocalDate])
  final case class SellResult(quantitySold: Double, proceedsMinor: Long, costBasisMinor: Long, realizedGainMinor: Long)
  final case class HoldingView(securityId: UUID, symbol: String, quantity: Double, costBasisMinor: Long, marketValueMinor: Long, unrealizedGainMinor: Long)
  ...
  private def hv(h: Holding): HoldingView = HoldingView(h.securityId, h.symbol, h.quantity, h.costBasisMinor, h.marketValueMinor, h.unrealizedGainMinor)
  ...
  def createSecurity(xa: Transactor[IO], p: Principal, r: CreateSecurityReq): IO[Out[SecurityView]] =
    ...
    else principal(p)(InvestmentRepo.createSecurity(r.symbol, r.name, r.currency.getOrElse("GBP"), r.assetClass.getOrElse("equity"))
      .map(id => SecurityView(id, r.symbol, r.name, r.currency.getOrElse("GBP"), r.assetClass.getOrElse("equity")))).transact(xa)
  ...
  def setPrice(xa: Transactor[IO], p: Principal, securityId: UUID, r: PriceReq): IO[Out[Ok]] = {
    val tx = for {
      ...
      res <-
        ...
        else InvestmentRepo.setPrice(securityId, r.priceMinor, r.asOf.getOrElse(LocalDate.now())).as(Right(Ok(securityId)): Out[Ok])
    } yield res
    tx.transact(xa)
  }
  ...
  def buy(xa: Transactor[IO], p: Principal, r: BuyReq): IO[Out[Ok]] =
    ...
    else principal(p)(InvestmentRepo.buy(p.userId, r.entityId, r.securityId, r.quantity, r.costBasisMinor, r.acquiredOn.getOrElse(LocalDate.now())).map(Ok)).transact(xa)
      ...
      val tx = for {
        ...
        res <-
          if (!a.can(Level.Admin, "wealth")) (Left(forbidden): Out[SellResult]).pure[ConnectionIO]
            ...
            if (available + 1e-9 < r.quantity) (Left(conflict(s"only $available units held")): Out[SellResult]).pure[ConnectionIO]
            else {
              val allocations = InvestmentService.fifoClose(lots, r.quantity)
              val on = r.on.getOrElse(LocalDate.now())
              allocations.traverse { case (lotId, qtyClosed, costClosed) =>
  ...
  val securitiesEndpoint   = sttp.tapir.endpoint.get.securityIn(bearer).in("api" / "investments" / "securities").errorOut(err).out(jsonBody[List[SecurityView]]).summary("Securities")
  val createSecEndpoint    = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "investments" / "securities").in(jsonBody[CreateSecurityReq]).errorOut(err).out(jsonBody[SecurityView]).summary("Add a security")
  val priceEndpoint        = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "investments" / "securities" / path[UUID]("id") / "price").in(jsonBody[PriceReq]).errorOut(err).out(jsonBody[Ok]).summary("Record a price")
  val buyEndpoint          = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "investments" / "lots").in(jsonBody[BuyReq]).errorOut(err).out(jsonBody[Ok]).summary("Buy (open a cost-basis lot)")
  val sellEndpoint         = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "investments" / "sell").in(jsonBody[SellReq]).errorOut(err).out(jsonBody[SellResult]).summary("Sell (FIFO close → realised gain)")
  val holdingsEndpoint     = sttp.tapir.endpoint.get.securityIn(bearer).in("api" / "investments" / "holdings").in(query[Option[UUID]]("entity")).errorOut(err).out(jsonBody[List[HoldingView]]).summary("Holdings (market value + unrealised gain)")

  def serverEndpoints(a: Auth, xa: Transactor[IO]): List[ServerEndpoint[Any, IO]] = List(
    securitiesEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (_: Unit) => securities(xa, p)),
    createSecEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (r: CreateSecurityReq) => createSecurity(xa, p, r)),
  ...
  val endpoints: List[AnyEndpoint] = List(securitiesEndpoint, createSecEndpoint, priceEndpoint, buyEndpoint, sellEndpoint, holdingsEndpoint)
}
