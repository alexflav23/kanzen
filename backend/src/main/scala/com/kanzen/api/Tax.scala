...
/** F38 — tax/VAT/deductibility estimation. Estimation only — Kanzen never files or pays.
  * Principal-private (gated on `ledger`, which is Manager/Staff-denied). */
object Tax {
  ...
  private val forbidden: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "forbidden", "tax estimates are Principal-only"))
  ...
  def incomeEstimate(xa: Transactor[IO], p: Principal, grossMinor: Long): IO[Out[IncomeEstimate]] = {
    ...
    else Authz.authorizer(p.role).map { authz =>
  ...
  val incomeEndpoint: Endpoint[String, Long, (StatusCode, ApiError), IncomeEstimate, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "tax" / "income-estimate").in(query[Long]("income")).errorOut(err).out(jsonBody[IncomeEstimate])
  ...
  val deductibleEndpoint: Endpoint[String, Unit, (StatusCode, ApiError), DeductibleReport, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "tax" / "deductible-report").errorOut(err).out(jsonBody[DeductibleReport])
      .summary("Deductible + VAT-reclaimable totals across approved expenses (Principal-only)")

  def serverEndpoints(a: Auth, xa: Transactor[IO]): List[ServerEndpoint[Any, IO]] = List(
    incomeEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (income: Long) => incomeEstimate(xa, p, income)),
