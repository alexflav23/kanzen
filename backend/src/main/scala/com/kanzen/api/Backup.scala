...
/** F30 — backup/export/restore. Principal-only. The archive is a self-descriptive JSON
  * document (manifest + dependency-ordered entity sections + per-section sha256). Restore
  * replays sections in dependency order and is id-preserving, so a delete→restore round-trips
  * faithfully. `age` encryption + S3 binary streaming + immutable snapshots are deferred. */
object Backup {
  ...
  private val forbidden: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "forbidden", "backup is Principal-only"))
  private def badReq(m: String): (StatusCode, ApiError) = (StatusCode.BadRequest, ApiError(400, "bad_request", m))
  private val invalid: (StatusCode, ApiError) = (StatusCode.UnprocessableEntity, ApiError(422, "invalid_archive", "Archive failed validation."))

  private def principalOnly[A](p: Principal)(q: ConnectionIO[A]): ConnectionIO[Out[A]] =
    Authz.authorizer(p.role).flatMap(a => if (a.can(Level.Admin, "backup")) q.map(Right(_): Out[A]) else (Left(forbidden): Out[A]).pure[ConnectionIO])
  ...
  def export(xa: Transactor[IO], p: Principal): IO[Out[ExportResult]] =
    principalOnly(p) {
      for {
        ...
        job      <- BackupRepo.recordExportJob(p.userId, manifest.hcursor.get[Json]("object_counts").getOrElse(Json.obj()), manifest, size)
        ...
        res <-
          if (!a.can(Level.Admin, "backup")) (Left(forbidden): Out[RestoreResult]).pure[ConnectionIO]
            ...
            if (!ok) (Left(invalid): Out[RestoreResult]).pure[ConnectionIO]
              ...
              val plan = BackupRepo.tables.flatMap(t => r.archive.data.get(t).map(j => t -> j.asArray.map(_.size.toLong).getOrElse(0L))).toMap
              BackupRepo.recordRestoreJob(p.userId, "dry_run", plan.asJson).as(Right(RestoreResult("dry_run", Map.empty, plan)): Out[RestoreResult])
            } else
              for {
                applied <- BackupRepo.tables.traverse { t =>
  ...
  val exportEndpoint   = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "backup" / "export").errorOut(err).out(jsonBody[ExportResult]).summary("Run a full export (self-descriptive archive + manifest)")
  val validateEndpoint = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "backup" / "validate").in(jsonBody[ValidateReq]).errorOut(err).out(jsonBody[ValidateResult]).summary("Validate an archive (checksums + schema compat)")
  val restoreEndpoint  = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "backup" / "restore").in(jsonBody[RestoreReq]).errorOut(err).out(jsonBody[RestoreResult]).summary("Restore (dry_run reports plan; full replays in dependency order)")
