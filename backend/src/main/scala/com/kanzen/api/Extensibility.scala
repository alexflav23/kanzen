...
/** F33 — extensibility: polymorphic tags, user-defined infinite taxonomies, and typed
  * custom-field definitions. Principal defines taxonomies/fields; Manager uses them
  * (tag/categorise); Staff read. Values live in each host entity's `attributes` and inherit
  * its permissions/scope (a `sensitive` field is stripped for non-Principal — [[Attributes]]). */
object Extensibility {
  ...
  final case class CustomFieldView(id: UUID, entityType: String, key: String, label: String, `type`: String, enumValues: Option[List[String]], sensitive: Boolean)
  final case class CreateFieldReq(entityType: String, key: String, label: String, `type`: String, enumValues: Option[List[String]], sensitive: Option[Boolean])
  ...
  private def cv(d: CustomFieldDef): CustomFieldView = CustomFieldView(d.id, d.entityType, d.key, d.label, d.`type`, enums(d.enumValues), d.sensitive)
  ...
  def entitiesWithTag(xa: Transactor[IO], p: Principal, tagId: UUID): IO[Out[List[TaggedEntity]]] =
    gate(p, Level.Read, "tag")(TagRepo.entitiesWithTag(tagId).map(_.map { case (t, i) => TaggedEntity(t, i) })).transact(xa)
  ...
  def createTaxonomy(xa: Transactor[IO], p: Principal, r: CreateTaxonomyReq): IO[Out[TaxonomyView]] =
    if (r.name.trim.isEmpty) IO.pure(Left(badReq("name required")))
    else gate(p, Level.Write, "taxonomy")(TaxonomyRepo.create(r.name, r.appliesTo).map(id => TaxonomyView(id, r.name, r.appliesTo, isSystem = false))).transact(xa)
  def nodes(xa: Transactor[IO], p: Principal, taxonomyId: UUID): IO[Out[List[NodeView]]] =
    gate(p, Level.Read, "taxonomy")(TaxonomyRepo.nodes(taxonomyId).map(_.map { case (id, par, n) => NodeView(id, par, n) })).transact(xa)
  def addNode(xa: Transactor[IO], p: Principal, taxonomyId: UUID, r: NodeReq): IO[Out[NodeView]] = {
    val tx = for {
      ...
      res <-
        ...
        else TaxonomyRepo.addNode(taxonomyId, r.parentId, r.name).map(id => Right(NodeView(id, r.parentId, r.name)): Out[NodeView])
    } yield res
    tx.transact(xa)
  }
  def linkTaxonomy(xa: Transactor[IO], p: Principal, r: TaxonomyLinkReq): IO[Out[Ok]] =
    if (!ENTITIES.contains(r.entityType)) IO.pure(Left(badReq(s"entityType must be one of ${ENTITIES.mkString(", ")}")))
    else gate(p, Level.Write, "taxonomy")(TaxonomyRepo.link(r.nodeId, r.entityType, r.entityId).as(Ok(true))).transact(xa)
  ...
  def createField(xa: Transactor[IO], p: Principal, r: CreateFieldReq): IO[Out[CustomFieldView]] =
    if (!ENTITIES.contains(r.entityType)) IO.pure(Left(badReq(s"entityType must be one of ${ENTITIES.mkString(", ")}")))
      ...
      gate(p, Level.Write, "custom_field")(
        CustomFieldRepo.create(p.userId, r.entityType, r.key, r.label, r.`type`, enumJson, r.sensitive.getOrElse(false)).map(cv)
      ).transact(xa)
    }
  ...
  val tagsEndpoint        = sttp.tapir.endpoint.get.securityIn(bearer).in("api" / "tags").errorOut(err).out(jsonBody[List[TagView]]).summary("All tags")
  val createTagEndpoint   = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "tags").in(jsonBody[CreateTagReq]).errorOut(err).out(jsonBody[TagView]).summary("Create a tag")
  val tagLinkEndpoint     = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "tag-links").in(jsonBody[TagLinkReq]).errorOut(err).out(jsonBody[Ok]).summary("Tag any entity (polymorphic)")
  val taggedEndpoint      = sttp.tapir.endpoint.get.securityIn(bearer).in("api" / "tags" / path[UUID]("id") / "entities").errorOut(err).out(jsonBody[List[TaggedEntity]]).summary("Entities carrying a tag")
  val taxonomiesEndpoint  = sttp.tapir.endpoint.get.securityIn(bearer).in("api" / "taxonomies").errorOut(err).out(jsonBody[List[TaxonomyView]]).summary("All taxonomies")
  val createTaxEndpoint   = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "taxonomies").in(jsonBody[CreateTaxonomyReq]).errorOut(err).out(jsonBody[TaxonomyView]).summary("Create a taxonomy (Manager+)")
  val nodesEndpoint       = sttp.tapir.endpoint.get.securityIn(bearer).in("api" / "taxonomies" / path[UUID]("id") / "nodes").errorOut(err).out(jsonBody[List[NodeView]]).summary("A taxonomy's nodes")
  val addNodeEndpoint     = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "taxonomies" / path[UUID]("id") / "nodes").in(jsonBody[NodeReq]).errorOut(err).out(jsonBody[NodeView]).summary("Add a node at any depth")
  val taxLinkEndpoint     = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "taxonomy-links").in(jsonBody[TaxonomyLinkReq]).errorOut(err).out(jsonBody[Ok]).summary("Categorise any entity under a node")
  val fieldsEndpoint      = sttp.tapir.endpoint.get.securityIn(bearer).in("api" / "custom-fields").in(query[String]("entityType")).errorOut(err).out(jsonBody[List[CustomFieldView]]).summary("Custom-field definitions for an entity type")
  val createFieldEndpoint = sttp.tapir.endpoint.post.securityIn(bearer).in("api" / "custom-fields").in(jsonBody[CreateFieldReq]).errorOut(err).out(jsonBody[CustomFieldView]).summary("Define a custom field (Principal)")
  val deleteFieldEndpoint = sttp.tapir.endpoint.delete.securityIn(bearer).in("api" / "custom-fields" / path[UUID]("id")).errorOut(err).out(jsonBody[Ok]).summary("Delete a custom-field definition")
  ...
  def serverEndpoints(a: Auth, xa: Transactor[IO]): List[ServerEndpoint[Any, IO]] = List(
    ...
    createTaxEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (r: CreateTaxonomyReq) => createTaxonomy(xa, p, r)),
    ...
    taxLinkEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (r: TaxonomyLinkReq) => linkTaxonomy(xa, p, r)),
    fieldsEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (et: String) => customFields(xa, p, et)),
    createFieldEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (r: CreateFieldReq) => createField(xa, p, r)),
    deleteFieldEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (id: UUID) => deleteField(xa, p, id)),
  )

  val endpoints: List[AnyEndpoint] = List(tagsEndpoint, createTagEndpoint, tagLinkEndpoint, taggedEndpoint,
    taxonomiesEndpoint, createTaxEndpoint, nodesEndpoint, addNodeEndpoint, taxLinkEndpoint,
    fieldsEndpoint, createFieldEndpoint, deleteFieldEndpoint)
}
