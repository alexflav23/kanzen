...
/** F19 — asset lifecycle events + timeline + lifetime-cost rollup. Reading the timeline
  * needs registry read (`asset`); logging an event needs `asset_event` write (Manager /
  * maintenance / Principal). Events are append-only history. */
object AssetEvents {
  private type Out[A] = Either[(StatusCode, ApiError), A]
  private val TYPES = Set("acquired", "serviced", "cleaned", "repaired", "moved", "appraised", "sold",
    "gifted", "lost", "stolen", "lent", "returned", "inspected", "restored", "note")

  final case class EventView(id: UUID, eventType: String, occurredAt: String, costMinor: Option[Long],
                             currency: Option[String], note: Option[String], party: Option[String])
  ...
  private def view(e: AssetEvent): EventView = EventView(e.id, e.eventType, e.occurredAt, e.costMinor, e.currency, e.note, e.party)

  private val forbidden: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "forbidden", "no access to asset events"))
  ...
  def timeline(xa: Transactor[IO], p: Principal, assetId: UUID): IO[Out[Timeline]] = {
    val tx = for {
      ...
      events <- if (authz.canRead("asset")) AssetEventRepo.timeline(assetId) else List.empty[AssetEvent].pure[ConnectionIO]
    ...
    if (!TYPES.contains(req.eventType)) IO.pure(Left(badReq(s"type must be one of ${TYPES.mkString(", ")}")))
      ...
      val tx = for {
        ...
        res <-
          ...
          else AssetEventRepo.add(assetId, req.eventType, req.costMinor, req.currency, req.note).map(e => Right(view(e)): Out[EventView])
      } yield res
      tx.transact(xa)
    }
  }
  ...
  val timelineEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), Timeline, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "assets" / path[UUID]("id") / "events").errorOut(err).out(jsonBody[Timeline])
  ...
  val logEndpoint: Endpoint[String, (UUID, LogReq), (StatusCode, ApiError), EventView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "assets" / path[UUID]("id") / "events").in(jsonBody[LogReq]).errorOut(err).out(jsonBody[EventView])
