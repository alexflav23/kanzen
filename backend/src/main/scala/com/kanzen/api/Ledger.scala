...
/** F18 / ADR-001 — the Postgres double-entry general ledger. Principal-private (ledger =
  * none for Manager/Staff). Postings are balanced-or-rejected; corrections are reversing
  * transactions (originals immutable). The UI never shows raw postings — only balances
...
object Ledger {
  ...
  final case class PostReq(kind: String, description: Option[String], occurredOn: Option[LocalDate], splits: List[SplitIn])
  final case class PostResult(transactionId: UUID)
  final case class BalanceResult(accountId: UUID, balanceMinor: Long)
  final case class RegisterRowView(transactionId: UUID, kind: String, description: Option[String], occurredOn: LocalDate, amountMinor: Long, memo: Option[String])
  final case class ReverseResult(reversalTransactionId: UUID)

  private val forbidden: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "forbidden", "the general ledger is Principal-only"))
  private val notFound: (StatusCode, ApiError)  = (StatusCode.NotFound, ApiError(404, "not_found", "No such transaction."))
  ...
  def createAccount(xa: Transactor[IO], p: Principal, req: AccountReq): IO[Out[AccountView]] = {
    val tx = Authz.authorizer(p.role).flatMap { authz =>
      ...
      else GlRepo.createAccount(p.userId, req.code, req.name, req.accountType, req.currency)
      ...
      val tx = for {
        ...
        res <-
          if (!authz.can(Level.Write, "ledger")) (Left(forbidden): Out[PostResult]).pure[ConnectionIO]
            ...
            for {
              t <- GlRepo.insertTransaction(p.userId, req.kind, req.description, req.occurredOn.getOrElse(LocalDate.now()), None)
    ...
    val tx = for {
      ...
      res <-
        if (!authz.can(Level.Write, "ledger")) (Left(forbidden): Out[ReverseResult]).pure[ConnectionIO]
          ...
          for {
            r <- GlRepo.insertTransaction(p.userId, "reversal", Some(s"Reversal of $txnId"), LocalDate.now(), Some(txnId))
  ...
  def register(xa: Transactor[IO], p: Principal, accountId: UUID): IO[Out[List[RegisterRowView]]] = {
    val tx = Authz.authorizer(p.role).flatMap { authz =>
      ...
      else GlRepo.register(accountId).map(rs =>
        Right(rs.map(r => RegisterRowView(r.transactionId, r.kind, r.description, r.occurredOn, r.amountMinor, r.memo))): Out[List[RegisterRowView]])
    }
    tx.transact(xa)
  }
  ...
  val accountEndpoint: Endpoint[String, AccountReq, (StatusCode, ApiError), AccountView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "ledger" / "accounts").in(jsonBody[AccountReq]).errorOut(err).out(jsonBody[AccountView]).summary("Create a ledger account (Principal-only)")

  val postEndpoint: Endpoint[String, PostReq, (StatusCode, ApiError), PostResult, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "ledger" / "transactions").in(jsonBody[PostReq]).errorOut(err).out(jsonBody[PostResult]).summary("Post a balanced transaction (Principal-only)")

  val reverseEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), ReverseResult, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "ledger" / "transactions" / path[UUID]("id") / "reverse").errorOut(err).out(jsonBody[ReverseResult]).summary("Reverse a transaction (immutable correction)")
  ...
  val balanceEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), BalanceResult, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "ledger" / "accounts" / path[UUID]("id") / "balance").errorOut(err).out(jsonBody[BalanceResult]).summary("Derived account balance (Principal-only)")

  val registerEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), List[RegisterRowView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "ledger" / "accounts" / path[UUID]("id") / "register").errorOut(err).out(jsonBody[List[RegisterRowView]]).summary("Account register (Principal-only)")
  ...
  val endpoints: List[AnyEndpoint] = List(accountEndpoint, postEndpoint, reverseEndpoint, balanceEndpoint, registerEndpoint)
}
