...
/** F22 — category templates: typed per-vertical attribute schemas that validate the
  * asset JSONB (wired into asset create) and drive the Specifications UI. */
object Templates {
  ...
  private val forbidden: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "forbidden", "no access to templates"))

  private def toJson(fields: List[FieldView]): Json =
    Json.arr(fields.map(f => Json.obj("key" -> Json.fromString(f.key), "type" -> Json.fromString(f.fieldType), "required" -> Json.fromBoolean(f.required))): _*)
  ...
  def schema(xa: Transactor[IO], p: Principal, vertical: String): IO[Out[List[FieldView]]] = {
    val tx = Authz.authorizer(p.role).flatMap { authz =>
      ...
      else TemplateRepo.schemaFor(vertical).map { s =>
        val fields = s.fold(List.empty[FieldView])(j => TemplateService.parseSchema(j).map(f => FieldView(f.key, f.fieldType, f.required)))
  ...
  def create(xa: Transactor[IO], p: Principal, req: CreateReq): IO[Out[List[FieldView]]] = {
    val tx = Authz.authorizer(p.role).flatMap { authz =>
      ...
      else TemplateRepo.create(req.verticalKey, req.name, toJson(req.fields)).as(Right(req.fields): Out[List[FieldView]])
    }
    tx.transact(xa)
  }
  ...
  val schemaEndpoint: Endpoint[String, String, (StatusCode, ApiError), List[FieldView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "templates" / path[String]("vertical")).errorOut(err).out(jsonBody[List[FieldView]])
  ...
  val createEndpoint: Endpoint[String, CreateReq, (StatusCode, ApiError), List[FieldView], Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "templates").in(jsonBody[CreateReq]).errorOut(err).out(jsonBody[List[FieldView]])
