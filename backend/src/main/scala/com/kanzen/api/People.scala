...
/** F10 — people/HR. Principal admin; Manager writes (HR ops); Staff read their **own**
  * record only (own-only enforced server-side). Manager/Principal see the roster, scoped
  * by property. Permit/review expiry is surfaced for the Inbox/dashboard (F11). */
object People {
  private type Out[A] = Either[(StatusCode, ApiError), A]

  final case class PersonView(id: UUID, userId: Option[UUID], name: String, role: Option[String],
                              jurisdiction: Option[String], propertyId: Option[UUID],
                              permitExpiry: Option[LocalDate], reviewDue: Option[LocalDate])
  final case class CreateReq(name: String, role: Option[String], jurisdiction: Option[String],
                             userId: Option[UUID], propertyId: Option[UUID],
                             permitExpiry: Option[LocalDate], reviewDue: Option[LocalDate])
  ...
  private val forbidden: (StatusCode, ApiError) = (StatusCode.Forbidden, ApiError(403, "forbidden", "no access to people"))
  ...
  def detail(xa: Transactor[IO], p: Principal, id: UUID): IO[Out[PersonView]] = {
    val tx = for {
      ...
      if (!authz.canRead("person")) Left(forbidden)
        ...
        case Some(per) =>
          if (p.role == "staff") {
            if (per.userId.contains(p.userId)) Right(view(per)) else Left(forbidden) // other staff records denied (AC3)
  ...
  def create(xa: Transactor[IO], p: Principal, req: CreateReq): IO[Out[PersonView]] = {
    val tx = Authz.authorizer(p.role).flatMap { authz =>
      if (!authz.can(Level.Write, "person")) (Left(forbidden): Out[PersonView]).pure[ConnectionIO]
        ...
        PeopleRepo.insert(p.userId, req.userId, req.name, req.role, req.jurisdiction, req.propertyId, req.permitExpiry, req.reviewDue)
  ...
  def expiring(xa: Transactor[IO], p: Principal, days: Int): IO[Out[List[PersonView]]] = {
    val tx = for {
      ...
      if (!authz.canRead("person")) Left(forbidden)
        ...
        val visible = rows.filter(per => if (p.role == "staff") per.userId.contains(p.userId) else per.propertyId.forall(scoped.contains))
  ...
  val listEndpoint: Endpoint[String, Unit, (StatusCode, ApiError), List[PersonView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "people").errorOut(err).out(jsonBody[List[PersonView]]).summary("The team roster (Staff see only themselves)")

  val expiringEndpoint: Endpoint[String, Int, (StatusCode, ApiError), List[PersonView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "people" / "expiring").in(query[Int]("days").default(60)).errorOut(err).out(jsonBody[List[PersonView]])
  ...
  val detailEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), PersonView, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "people" / path[UUID]("id")).errorOut(err).out(jsonBody[PersonView]).summary("A person's record")
  ...
  val createEndpoint: Endpoint[String, CreateReq, (StatusCode, ApiError), PersonView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "people").in(jsonBody[CreateReq]).errorOut(err).out(jsonBody[PersonView]).summary("Add a person (Manager+)")
