...
/** F03 — the typed nested **location tree** under a property (rooms → areas → cabinets →
  * shelves …). Reads need property read + scope; writes need property write (Manager+;
  * Staff can't edit locations — AC5). Enforces tree integrity: a parent must be in the
  * same property, no node may move under its own descendant (cycle), and a node with
  * live children can't be deleted (AC2/AC3). */
object Locations {
  private type Out[A] = Either[(StatusCode, ApiError), A]

  final case class LocationView(id: UUID, parentId: Option[UUID], kind: String, name: String,
                                floor: Option[String], area: Option[String], notes: Option[String])
  final case class CreateReq(propertyId: UUID, parentId: Option[UUID], kind: String, name: String,
                             floor: Option[String], area: Option[String], notes: Option[String])
  ...
  private def descendantsOf(all: List[Location], rootId: UUID): Set[UUID] = {
    ...
    @annotation.tailrec def loop(frontier: List[UUID], acc: Set[UUID]): Set[UUID] = frontier match {
      case Nil    => acc
  ...
  def create(xa: Transactor[IO], p: Principal, req: CreateReq): IO[Out[LocationView]] = {
    val tx = authorizeWrite(p, req.propertyId).flatMap {
      ...
      case Right(_) =>
        ...
        parentOk.flatMap {
          case false => (Left(badReq("parent must be a location in the same property")): Out[LocationView]).pure[ConnectionIO]
          case true =>
            PropertyRepo
              .insertLocation(p.userId, req.propertyId, req.parentId, req.kind, req.name, req.floor, req.area, req.notes)
            ...
            req.newParentId match {
              ...
              case Some(np) =>
                PropertyRepo.locations(loc.propertyId).flatMap { all =>
                  if (!all.exists(_.id == np)) (Left(badReq("new parent must be in the same property")): Out[LocationView]).pure[ConnectionIO]
                  else if (descendantsOf(all, id).contains(np)) (Left(badReq("cannot move a location under its own descendant")): Out[LocationView]).pure[ConnectionIO]
  ...
  val treeEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), List[LocationView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "properties" / path[UUID]("id") / "locations").errorOut(err).out(jsonBody[List[LocationView]])
  ...
  val createEndpoint: Endpoint[String, CreateReq, (StatusCode, ApiError), LocationView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "locations").in(jsonBody[CreateReq]).errorOut(err).out(jsonBody[LocationView])
  ...
  val patchEndpoint: Endpoint[String, (UUID, PatchReq), (StatusCode, ApiError), LocationView, Any] =
    sttp.tapir.endpoint.patch.securityIn(auth.bearer[String]())
      .in("api" / "locations" / path[UUID]("id")).in(jsonBody[PatchReq]).errorOut(err).out(jsonBody[LocationView])
      .summary("Rename / edit a location (Manager+)")

  val moveEndpoint: Endpoint[String, (UUID, MoveReq), (StatusCode, ApiError), LocationView, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "locations" / path[UUID]("id") / "move").in(jsonBody[MoveReq]).errorOut(err).out(jsonBody[LocationView])
  ...
  val deleteEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), DeletedResp, Any] =
    sttp.tapir.endpoint.delete.securityIn(auth.bearer[String]())
      .in("api" / "locations" / path[UUID]("id")).errorOut(err).out(jsonBody[DeletedResp])
