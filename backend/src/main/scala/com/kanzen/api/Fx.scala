...
/** F37 — currencies & FX overlay. Native amounts are the truth; this layer reads daily snapshots and converts at the
  * rate on a chosen date (default today), **always labelled** with the rate basis. Cross pairs triangulate via the base
  * (GBP). Rollups return a per-currency breakdown (native = primary) plus a single dated-estimate total.
  */
object Fx {
  ...
  private val noRate: (StatusCode, ApiError) =
    (StatusCode.UnprocessableEntity, ApiError(422, "no_rate", "No FX rate available for that pair/date."))
            ...
            case Some(rate) =>
              Right(
                Conversion(
                  r.amountMinor,
                  r.from,
                  FxService.convertMinor(r.amountMinor, rate),
                  r.to,
                  rate,
                  on,
                  label(rate, on)
    ...
    Authz
      ...
      .flatMap { a =>
        if (!a.can(Actions.byKey("fx:view"))) (Left(forbidden): Out[Rollup]).pure[ConnectionIO]
          ...
          byCcy.toList
            .traverse { case (ccy, native) =>
              FxRepo.crossRateOn(ccy, r.target, on).map(rate => (ccy, native, rate))
            }
            .map { rows =>
              val converted = rows.collect { case (c, n, Some(rate)) =>
                BreakdownRow(c, n, FxService.convertMinor(n, rate), rate)
              }
              ...
              Right(
                Rollup(
                  ...
                  total,
                  on,
                  s"≈ ${r.target} dated estimate at $on rate",
                  converted.sortBy(_.currency),
                  missing
                )
              ): Out[Rollup]
            }
  ...
  val currenciesEndpoint = sttp.tapir.endpoint.get
    .securityIn(bearer)
    .in("api" / "currencies")
    .errorOut(err)
    .out(jsonBody[List[CurrencyView]])
    .summary("Active currencies")
  val convertEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "fx" / "convert")
    .in(jsonBody[ConvertReq])
    .errorOut(err)
    .out(jsonBody[Conversion])
    .summary("Convert an amount at the rate on a date")
  val rollupEndpoint = sttp.tapir.endpoint.post
    .securityIn(bearer)
    .in("api" / "fx" / "rollup")
    .in(jsonBody[RollupReq])
    .errorOut(err)
    .out(jsonBody[Rollup])
