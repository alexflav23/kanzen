...
/** F21 — warranty, provenance & insurance on assets. Warranties are registry-managed
  * (Manager+ write). Insurance carries the Principal-private `insured_value`, so it is
  * Principal-only (Manager denied via the field-level Authorizer). */
object Provenance {
  private type Out[A] = Either[(StatusCode, ApiError), A]

  final case class WarrantyView(id: UUID, provider: Option[String], startsOn: Option[LocalDate], endsOn: Option[LocalDate])
  final case class AddWarrantyReq(provider: Option[String], startsOn: Option[LocalDate], endsOn: Option[LocalDate])
  final case class InsuranceView(insured: Boolean, policyRef: Option[String], insurer: Option[String],
                                 insuredValueMinor: Option[Long], renewalOn: Option[LocalDate])
  final case class SetInsuranceReq(insured: Boolean, policyRef: Option[String], insurer: Option[String],
                                   insuredValueMinor: Option[Long], renewalOn: Option[LocalDate])
  ...
  def addWarranty(xa: Transactor[IO], p: Principal, assetId: UUID, req: AddWarrantyReq): IO[Out[OkResult]] = {
    val tx = for {
      ...
      res <-
        ...
        else InsuranceRepo.addWarranty(assetId, req.provider, req.endsOn, req.startsOn).as(Right(OkResult(true)): Out[OkResult])
    } yield res
    tx.transact(xa)
  }
  ...
  def getInsurance(xa: Transactor[IO], p: Principal, assetId: UUID): IO[Out[InsuranceView]] = {
    val tx = Authz.authorizer(p.role).flatMap { authz =>
      if (!authz.can(Level.Read, "asset", Some("insured_value"))) (Left(forbidden): Out[InsuranceView]).pure[ConnectionIO]
      else InsuranceRepo.get(assetId).map {
        case Some(i) => Right(InsuranceView(i.insured, i.policyRef, i.insurer, i.insuredValueMinor, i.renewalOn)): Out[InsuranceView]
  ...
  def setInsurance(xa: Transactor[IO], p: Principal, assetId: UUID, req: SetInsuranceReq): IO[Out[OkResult]] = {
    val tx = for {
      ...
      res <-
        ...
        else InsuranceRepo.set(assetId, req.insured, req.policyRef, req.insuredValueMinor, req.renewalOn, req.insurer).as(Right(OkResult(true)): Out[OkResult])
    } yield res
    tx.transact(xa)
  }
  ...
  val warrantiesEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), List[WarrantyView], Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "assets" / path[UUID]("id") / "warranties").errorOut(err).out(jsonBody[List[WarrantyView]]).summary("Asset warranties")
  ...
  val addWarrantyEndpoint: Endpoint[String, (UUID, AddWarrantyReq), (StatusCode, ApiError), OkResult, Any] =
    sttp.tapir.endpoint.post.securityIn(auth.bearer[String]())
      .in("api" / "assets" / path[UUID]("id") / "warranties").in(jsonBody[AddWarrantyReq]).errorOut(err).out(jsonBody[OkResult]).summary("Add a warranty (Manager+)")

  val getInsuranceEndpoint: Endpoint[String, UUID, (StatusCode, ApiError), InsuranceView, Any] =
    sttp.tapir.endpoint.get.securityIn(auth.bearer[String]())
      .in("api" / "assets" / path[UUID]("id") / "insurance").errorOut(err).out(jsonBody[InsuranceView]).summary("Asset insurance (Principal-only)")

  val setInsuranceEndpoint: Endpoint[String, (UUID, SetInsuranceReq), (StatusCode, ApiError), OkResult, Any] =
    sttp.tapir.endpoint.put.securityIn(auth.bearer[String]())
      .in("api" / "assets" / path[UUID]("id") / "insurance").in(jsonBody[SetInsuranceReq]).errorOut(err).out(jsonBody[OkResult]).summary("Set asset insurance (Principal-only)")

  def serverEndpoints(a: Auth, xa: Transactor[IO]): List[ServerEndpoint[Any, IO]] = List(
    warrantiesEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (id: UUID) => warranties(xa, p, id)),
    addWarrantyEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => { case (id, r) => addWarranty(xa, p, id, r) }),
    getInsuranceEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => (id: UUID) => getInsurance(xa, p, id)),
    setInsuranceEndpoint.serverSecurityLogic(a.securityLogic).serverLogic(p => { case (id, r) => setInsurance(xa, p, id, r) }),
  )

  val endpoints: List[AnyEndpoint] = List(warrantiesEndpoint, addWarrantyEndpoint, getInsuranceEndpoint, setInsuranceEndpoint)
}
